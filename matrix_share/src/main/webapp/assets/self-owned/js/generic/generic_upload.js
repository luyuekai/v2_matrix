function GenericUploadPageViewModel(userCode,systemCode,moduleCode,functionCode) {
  userCode = userCode||'scripter_user';
  systemCode = systemCode || 'scripter_system';
  moduleCode = moduleCode || 'scripter_module';
  functionCode = functionCode || 'scripter_function';
  var self = this;
  self.uploadRef = null;
...
var initialize_upload_environment = function(file_upload_component_id,vm,successListener,failerListener,allowedFileExtensions,allowedPreviewTypes,allowedPreviewMimeTypes) {
  ...
  $upload.fileinput({
    ...
    allowedPreviewTypes: allowedPreviewTypes, // disable preview of standard types
    allowedPreviewMimeTypes: allowedPreviewMimeTypes, // allow content to be shown only for certain mime types
